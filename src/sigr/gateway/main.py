...
import threading
import serial
# import time # F401: 'time' imported but unused (ctime, sleep are directly imported)
import json
    ...
    while True:
        if recv.strip() == '':
            ...
            if int(json_lora.get("ID")) == 1 :  # Device ID-1 existed in gateway database
                ...
                print('init_device')  # noqa: T201
                # init_device()  #Create sqlite table for device 1.
            recv = ''
        print("This is %s. %s" % (func, ctime()))  # noqa: T201
        sleep(1)
...
def gateway_mqtt(func):
    while True:
        ...
        print('Waiting for MQTT data from server.....')  # noqa: T201
...
def gateway_init():
    print('gateway init')  # noqa: T201
    print('check gateway database existed or not')  # noqa: T201
    print('dateway database do not exist')  # noqa: T201
    print('read gateway ID from gateway.inf')  # noqa: T201
    print('send ID to server to check gateway database backup on server or not')  # noqa: T201
    # requests.post('http://www.sips.com/gateway', data=json.dumps({'ID': '123456'}))
    print('if ID backup on server, download it, otherwise init it')  # noqa: T201
    # url = 'http://www.sips.com/gateway/123456/sips.db'
    # r = requests.get(url)
    # with open("sips.db", "wb") as code:
    # code.write(r.content)
    print('init database......')  # noqa: T201
...
if __name__ == '__main__':
    ...
    for t in threads:
        #        t.setDaemon(True)
        t.start()
    while True:  # This was line 87 in the original
        print("\nThis is the main thread!")  # noqa: T201 # Line 88, now correctly indented
        sleep(2)  # Line 89, now correctly indented
